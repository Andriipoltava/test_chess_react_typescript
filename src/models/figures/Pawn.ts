...
export class Pawn extends Figure {
    ...
    canMove(target: Cell): boolean {
        ...
        if ((target.y === this.cell.y + dir || this.isFirstStep && (target.y === this.cell.y + firstStepDir))
            && target.x === this.cell.x && this.cell.board.getCell(target.x, target.y).isEmpty()) {
            return true
        }
        if (target.y === this.cell.y + dir && (target.x === this.cell.x + 1|| target.x === this.cell.x - 1)
